...
module.exports.sockets = {
	...
	// connects
	// (To control whether a socket is allowed to connect, check out
	// `authorization` config.)
	// Keep in mind that Sails' RESTful simulation for sockets
	// mixes in socket.io events for your routes and blueprints automatically.
	onConnect : function(session, socket) {
		// By default: do nothing
		// This is a good place to subscribe a new socket to a room, inform
		// other users that
	...
	// It's a powerful, efficient approach, but it involves a tiny bit of
	// planning.
	// At scale, you'll want to be able to copy your app onto multiple Sails.js
	// servers
	// and throw them behind a load balancer.
	//
	// One of the big challenges of scaling an application is that these sorts
	// of clustered
	// deployments cannot share memory, since they are on physically different
	// machines.
	// On top of that, there is no guarantee that a user will "stick" with the
	// same server between
	// requests (whether HTTP or sockets), since the load balancer will route
	// each request to the
	// Sails server with the most available resources. However that means that
	// all room/pubsub/socket
	// processing and shared memory has to be offloaded to a shared, remote
	...
	//
	// By default (`authorization: true`), when a socket tries to connect, Sails
	// verifies
	...
	//
	// If you don't care about keeping track of your socket users between
	// requests,
	// you can bypass this cookie check by setting `authorization: false`
	// which will disable the session for socket requests (req.session is still
	// accessible
	// in each request, but it will be empty, and any changes to it will not be
	// persisted)
	//
	...
	// Primarily because of this situation, as well as a handful of other
	// advanced
	 ...
	 *  // // to allow the connection, call `cb(null, true)` // to prevent the
	 * connection, call `cb(null, false)` // to report an error, call `cb(err)` }
	 */
	authorization : true,
	...
	// When client closes connection, the # of seconds to wait before attempting
	// a reconnect.
	...
	// The max # of seconds to wait for an expcted heartbeat before declaring
	// the pipe broken
	...
	//
	// If you are using a hosting provider that doesn't allow you to start
	// servers
	// other than on port 80 or the provided port, and you still want to support
	// flashsockets
	// you can set the `flash policy port` to -1
	'flash policy port' : 10843,

	// Used by the HTTP transports. The Socket.IO server buffers HTTP request
	// bodies up to this limit.
	...
	// Adds a Cache-Control: private, x-gzip-ok="", max-age=31536000 header to
	// static requests,
	// but only if the file is requested with a version number like
	// /socket.io/socket.io.v0.9.9.js.
	'browser client expires' : 315360000,

	// Does Socket.IO need to GZIP the static files?
	// This process is only done once and the computed output is stored in
	// memory.
	...
	// Should be set to true when you want the location handshake to match the
	// protocol of the origin.
	...
	// A Static instance that is used to serve the socket.io client and its
	// dependencies.
